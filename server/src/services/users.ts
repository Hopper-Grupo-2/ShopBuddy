...
export default class UsersServices {
  ...
  public static async signupUser(user: IUser) {
    try {
      const foundUserByEmail = await this.Repository.getUserByEmail(user.email);
  ...
  public static async updateUser(
    ...
    try {
      ...
      if (foundUserByUsername !== null && user.username !== oldUser.username)
        throw ErrorHandler.createError(
          "Conflict",
          "This username is already in use"
        );
      ...
      if (await bcrypt.compare(plainTextPassword, oldUser.password)) {
        ...
        const updatedUser = await this.Repository.updateUser(userId, userInfo);
        return updatedUser;
      } else {
        throw ErrorHandler.createError(
          "UnauthorizedError",
    ...
    try {
      ...
      for (const list of userLists) {
        if (list.members.length === 1) {
          await this.ListRepository.deleteList(list._id.toString());
        } else {
